      ...
      <main className="flex-grow px-container-margin max-w-7xl mx-auto w-full pb-section-gap pt-16 md:pt-20">
        ...
        >
          <div className="flex items-center gap-4 mb-4">
            <Link to="/shop" className="text-secondary hover:text-primary transition-colors flex items-center gap-1 font-label-caps text-label-caps uppercase">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-gutter">
          <div className="lg:col-span-8 flex flex-col border-t border-primary">
            {cart.length === 0 ? (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="py-12 md:py-16 flex flex-col items-center gap-4"
              >
              ...
              cart.map((item, idx) => (
                ...
                >
                  <button
                    ...
                    className="absolute top-6 md:top-8 right-0 text-secondary hover:text-primary transition-colors opacity-0 group-hover:opacity-100 z-10"
                  >
                    <span className="material-symbols-outlined">close</span>
                  </button>
                  <div className="w-full sm:w-32 md:w-48 aspect-[3/4] bg-surface-container overflow-hidden flex-shrink-0">
                    <img
                      alt={item.name}
                  ...
                  <div className="flex flex-col justify-between flex-grow pt-1 md:pt-2">
                    <div>
                      <div className="flex justify-between items-start mb-2 pr-8">
                        <h3 className="font-headline-md text-[20px] md:text-headline-md leading-none uppercase">{item.name}</h3>
                        <span className="font-body-lg text-body-lg">${item.price}</span>
                      </div>
                    ...
                    <div className="flex items-center justify-between mt-4 md:mt-8">
                      <div className="flex items-center border border-primary font-label-caps text-label-caps w-fit">
                        <button
                          ...
                          className="px-3 py-1 hover:bg-primary hover:text-surface transition-colors"
                          aria-label="Decrease"
                        >
                          -
                        </button>
                      ...
                      <button className="font-label-caps text-label-caps underline hover:text-secondary transition-colors">
                        EDIT DETAILS
                      </button>
                    </div>
                  </div>
          ...
          <div className="lg:col-span-4 mt-8 lg:mt-0">
            <motion.div
              ...
              className="sticky top-24 md:top-28 border border-primary p-5 md:p-8 flex flex-col gap-5 md:gap-6"
